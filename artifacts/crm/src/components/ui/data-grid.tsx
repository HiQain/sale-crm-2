...
import { MoreHorizontal, ArrowUpDown, ChevronDown, ChevronUp, Search, Plus, Trash2 } from "lucide-react";
...
export function DataGrid<T>({ 
  ...
  return (
    <div className="flex flex-col h-full bg-card rounded-lg border border-border shadow-sm overflow-hidden">
                    ...
                    {columns.map(col => {
                      ...
                      return (
                        <TableCell 
                          ...
                          onDoubleClick={(e) => {
                            e.stopPropagation();
                            if (col.editable !== false && onEdit) {
                              startEdit(row, col.key as string, value);
                            }
                          }}
                        >
                          {isEditing ? (
                    ...
                    <TableCell className="px-4 py-2 text-right opacity-0 group-hover:opacity-100 transition-opacity">
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end" className="w-40">
                          {onEdit && (
                            <DropdownMenuItem onClick={(e) => { e.stopPropagation(); /* Could open full edit form here */ }}>
                              Edit Details
                            </DropdownMenuItem>
                          )}
                          {onDelete && (
      ...
      {/* Footer / Pagination (Mocked for now) */}
